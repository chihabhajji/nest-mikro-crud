...
import {FilterOperator, FilterQueryParam, OrderQueryParam, ScalarPath, walkPath,} from "..";
    ...
    async parseOrder({
        ...
        order.forEach((raw) => {
            ...
            walkPath(
                orderOptions,
                path,
                (obj: QueryOrderMap<Entity>, key: string) => (obj[key as unknown as keyof QueryOrderMap<Entity>] = order)
            );
        });
        return orderOptions;
    }
